...
let checkOddEven = function(number) {
    let status = ""
...
function loopCheckOddEven(limit) {
    let keterangan = ""
...
const isPalindrome = (inputUser) => {
    let cleanStr = ""
    let hurufKecil = inputUser.toLowerCase();

  for (let i = 0; i < hurufKecil.length; i++) {
    if (hurufKecil[i] !== " ") {
      cleanStr = cleanStr + hurufKecil[i];
    }
  }
